...
This software package provides the basic functions to start using
semantic similarity measures directly from a rdf or owl file.

A web tool using this package is available at:
...
To create the semantic base file (*metals.db*) from the *metals.owl*
file:
...
Using the python function directly (first download metals.db and
metals.txt from this repository):
...
and *maltose catabolic process* execute:
...
`Q12346 <http://www.uniprot.org/uniprot/Q12346>`__ execute:
...
-  semanticbase.py : provides a function to produce the semantic-base as
   a SQLite database

-  ssm.py : provides the functions to calculate semantic similarity
   based on the SQLite database

-  annotations.py : provides the functions to get the annotations for
   the given proteins
...
-  F. Couto and A. Lamurias, “Semantic similarity definition,” in
   Encyclopedia of Bioinformatics and Computational Biology (S.
   Ranganathan, K. Nakai, C. Schönbach, and M. Gribskov, eds.), vol. 1,
   pp. 870–876, Oxford: Elsevier, 2019
   [https://doi.org/10.1016/B978-0-12-809633-8.20401-9][https://www.researchgate.net/publication/323219905\_Semantic\_Similarity\_Definition]
